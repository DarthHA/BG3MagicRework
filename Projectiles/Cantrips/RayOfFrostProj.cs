...
{
    public class RayOfFrostProj : BaseMagicProj
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] == 1)
            {
                ...
                for (int i = 0; i < 10; i++)
                {
                    Vector2 ShootVel = (Main.rand.NextFloat() * MathHelper.TwoPi).ToRotationVector2() * Main.rand.NextFloat() * 6;
                    smokeParticles.NewParticle(TargetPos, ShootVel, Main.rand.NextFloat() * 0.15f + 0.15f, Color.LightBlue);
                }
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            DrawUtils.DrawRoSLaser(texRibbon, bars2, Color.Yellow * a1, 0.1f, 1f, -Projectile.ai[0] / 120f, DrawUtils.ReverseSubtract);
            DrawUtils.DrawRoSLaser(texRibbon, bars1, Color.Cyan * a1, 0.1f, 1f, -Projectile.ai[0] / 120f, BlendState.Additive);
            DrawUtils.DrawRoSLaser(texRibbon, bars1, Color.White * a1, 0.1f, 1f, -Projectile.ai[0] / 120f, BlendState.Additive);
            DrawUtils.DrawTrail(texBlobGlow2, bars2, Color.White * a2, BlendState.Additive);

            EasyDraw.AnotherDraw(BlendState.NonPremultiplied);
            smokeParticles.DrawParticle(false);

            EasyDraw.AnotherDraw(BlendState.Additive);
            Particles.DrawParticle(tex, Color.Cyan, true, new Vector2(2, 1));
            ...
            if (Projectile.ai[0] <= 10)
            {
                ...
                Main.spriteBatch.Draw(texBloomFlare, TargetPos - Main.screenPosition, null, Color.LightBlue, 0, texBloomFlare.Size() / 2f, 0.15f * a3, SpriteEffects.None, 0);
            ...
            if (Projectile.ai[0] <= 10)
            {
                ...
                Main.spriteBatch.Draw(texHollowCircleSoftEdge, TargetPos - Main.screenPosition, null, Color.LightBlue * light, 0, texHollowCircleSoftEdge.Size() / 2f, scale * 0.15f, SpriteEffects.None, 0);
            }
        ...
        {
            target.DeepAddCCBuff(ModContent.BuffType<IceSlowBuff>(), GetTimeSpan<RayOfFrostSpell>() * 60);
        }
