...
{
    public class ZapinatorAttackProj : BaseMagicProj
    {
        ...
        {
            if (CurrentColor == null) CurrentColor = Main.DiscoColor;
            if (Projectile.ai[0] == 0)
            {
                Projectile.ai[1]++;
                Projectile.rotation = Projectile.velocity.ToRotation();
                if (TravelDistance > GetSpellRange<ZapinatorAttackSpell>() * 16f)
                {
                    Projectile.localAI[0] = 1;
                }
                if ((Collision.SolidTiles(Projectile.position, Projectile.width, Projectile.height) && !CarefulSpellMM)
                   || TravelDistance > GetSpellRange<ZapinatorAttackSpell>() * 16f * 2f)
                {
                    Projectile.ai[0] = 1;
                    Projectile.ai[1] = 0;
        ...
        public override void SafeModifyHit(NPC target, ref NPC.HitModifiers modifiers, ref DiceDamage diceUsed, ref float damageModifier, ref Dictionary<DamageElement, float> resistance)
        {
            ...
            {
                diceUsed.ChangeUnknownElement((DamageElement)Main.rand.Next(10) + 1);
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (Projectile.ai[0] == 0)
            {
                ...
                Main.spriteBatch.Draw(texExtra, Projectile.Center - Main.screenPosition, null, CurrentColor ?? Color.White, Projectile.rotation, texExtra.Size() / 2f, new Vector2(1, 0.5f), SpriteEffects.None, 0);
            ...
            else if (Projectile.ai[0] == 1)
            {
                ...
                if (Projectile.ai[1] <= 20)
                {
                    ...
                    Main.spriteBatch.Draw(LightTex, Projectile.Center - Main.screenPosition, null, CurrentColor.Value * light, Projectile.rotation, LightTex.Size() / 2f, 0.03f * scale, SpriteEffects.None, 0);
                ...
                Particles.DrawParticle(tex, CurrentColor ?? Color.White, true, new Vector2(1, 1));
                EasyDraw.AnotherDraw(BlendState.AlphaBlend);
            }
            return false;
        }

        public DamageElement GetElement(Color color)
        {
            DamageElement? result = null;
            float a = 0;
            for (int i = 0; i < 12; i++)
            {
                if (result == null)
                {
                    result = (DamageElement)i;
                    a = Vector3.Dot(SomeUtils.GetColor((DamageElement)i).ToVector3(), color.ToVector3()) / (color.ToVector3().Length() * SomeUtils.GetColor((DamageElement)i).ToVector3().Length());
                }
                else
                {
                    float tmp = Vector3.Dot(SomeUtils.GetColor((DamageElement)i).ToVector3(), color.ToVector3()) / (color.ToVector3().Length() * SomeUtils.GetColor((DamageElement)i).ToVector3().Length());
                    if (tmp < a)
                    {
                        result = (DamageElement)i;
                        a = tmp;
