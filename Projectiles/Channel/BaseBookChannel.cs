...
{
    public abstract class BaseBookChannel : BaseChannel              //戏法按照1.5秒算
    {
        //未处理的目标位置
        ...
        //书的横向相对坐标
        public float BookOffSet = 0;
        //翻页角度
        public float PageRot = 0;
        //内页翻页角度
        public float PageRot2 = 0;
        ...
        public override void AI()
        {
            ...
            if (Phase == 0) //20帧抬手
            {
                ...
                if (Timer < 10)
                {
                    BookOffSet = MathHelper.Lerp(0, 20, SmoothSwing(Timer / 10f));
                    PageRot = 0;
                }
                else
                {
                    BookOffSet = 20;
                    PageRot = MathHelper.Lerp(0, MathHelper.Pi / 3f, (Timer - 10) / 10f);
                }
            ...
            else if (Phase == 1)  //持续到单击待机，期间可以改环
            {
                ...
                BookOffSet = 20;
                PageRot = MathHelper.Pi / 3f;
                PageRot2 += 0.05f;
                if (PageRot2 > 1) PageRot2--;
            ...
            else if (Phase == 4)  //持续15帧后摇
            {
                ...
                if (P4TimeLeft >= 15)
                {
                    ...
                    if (Timer < P4TimeLeft - 10)
                    {
                        BookOffSet = 20;
                        PageRot = MathHelper.Pi / 3f;
                    }
                    else if (Timer < P4TimeLeft - 5)
                    {
                        BookOffSet = 20;
                        PageRot = MathHelper.Lerp(MathHelper.Pi / 3f, 0, MathHelper.Clamp((Timer - (P4TimeLeft - 10)) / 5f, 0f, 1f));
                    }
                    else
                    {
                        BookOffSet = MathHelper.Lerp(20, 5, MathHelper.Clamp((Timer - (P4TimeLeft - 5)) / 5f, 0f, 1f));
                        PageRot = 0;
                    }
                ...
                else
                {
                    ...
                    BookOffSet = 20;
                    PageRot = MathHelper.Lerp(MathHelper.Pi / 3f, 0, Timer / (float)P4TimeLeft);
                    if (Timer < P4TimeLeft / 3f)
                    {
                        BookOffSet = 20;
                        PageRot = MathHelper.Pi / 3f;
                    }
                    else if (Timer < P4TimeLeft / 3f * 2f)
                    {
                        BookOffSet = 20;
                        PageRot = MathHelper.Lerp(MathHelper.Pi / 3f, 0, (Timer - P4TimeLeft / 3f) / (P4TimeLeft / 3f));
                    }
                    else
                    {
                        BookOffSet = MathHelper.Lerp(20, 5, (Timer - P4TimeLeft / 3f * 2f) / (P4TimeLeft / 3f));
                        PageRot = 0;
                    }
                }
                owner.itemRotation = (float)Math.Atan2(unit.Y * owner.direction, unit.X * owner.direction);
            ...
            else if (Phase == 5)       //持续5帧撤回复位
            {
                ...
                BookOffSet = 20;
                PageRot = MathHelper.Lerp(MathHelper.Pi / 3f, 0, Timer / 5f);
                if (PageRot2 < 1)
                {
                    PageRot2 += 0.2f;
                }
            ...
            else if (Phase == 6) //10帧放手
            {
                ...
                BookOffSet = MathHelper.Lerp(20, 5, Timer / 10f);
                PageRot = 0;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            DrawUtils.DrawBooks(TextureLibrary.Book, owner.Center + new Vector2(BookOffSet * owner.direction, -5), owner.direction, -PageRot, 1f, lightColor);
            if (PageRot2 != 0 && PageRot2 != 1) //翻书特效
            {
                float rot = MathHelper.Lerp(PageRot, -PageRot, PageRot2);
                DrawUtils.DrawBooks(TextureLibrary.Book, owner.Center + new Vector2(BookOffSet * owner.direction, -5), owner.direction, rot, 1f, lightColor);
            }
            DrawUtils.DrawBooks(WeaponTex, owner.Center + new Vector2(BookOffSet * owner.direction, -5), owner.direction, PageRot, 1f, lightColor);
    ...
    public class NormalBookChannel : BaseBookChannel
    {
        ...
        {
            int protmp = Projectile.NewProjectile(owner.GetSource_FromThis("BG3Magic"), owner.Center, Vector2.Zero, ModContent.ProjectileType<NormalBookChannel>(), 0, 0, owner.whoAmI);
            if (protmp >= 0 && protmp < 1000)
            {
                NormalBookChannel modproj = Main.projectile[protmp].ModProjectile as NormalBookChannel;
