...
{
    public abstract class BaseRepeaterChannel : BaseChannel          //戏法按0.5秒算
    {
    ...
    public class NormalRepeaterChannel : BaseRepeaterChannel
    {
        ...
        {
            int protmp = Projectile.NewProjectile(owner.GetSource_FromThis("BG3Magic"), owner.Center, Vector2.Zero, ModContent.ProjectileType<NormalRepeaterChannel>(), 0, 0, owner.whoAmI);
            if (protmp >= 0 && protmp < 1000)
            {
                NormalRepeaterChannel modproj = Main.projectile[protmp].ModProjectile as NormalRepeaterChannel;
