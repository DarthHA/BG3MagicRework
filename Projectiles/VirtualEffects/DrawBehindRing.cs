...
{
    public class DrawBehindRing : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Projectile source = Main.projectile[(int)Projectile.ai[0]];
            BaseChannel modproj = source.ModProjectile as BaseChannel;
            EverythingLibrary.spells[modproj.Spell].DrawBehind(owner, modproj.currentRing, modproj.Light, modproj.LightColor, modproj.Scale, modproj.miscTimer);
            Texture2D tex = TextureLibrary.Ring1;
        ...
        public static void KillThis(int wmi)
        {
            ...
            {
                if (proj.type == ModContent.ProjectileType<DrawBehindRing>() && proj.ai[0] == wmi)
                {
                    proj.Kill();
                }
            }
        ...
        {
            int protmp = Projectile.NewProjectile(Main.LocalPlayer.GetSource_FromThis("BG3Magic"), Main.projectile[wmi].Center, Vector2.Zero, ModContent.ProjectileType<DrawBehindRing>(), 0, 0, Main.projectile[wmi].owner, wmi);
            return protmp;
        }
