...
{
    public class CallLightningSpell : BaseSpell
    {
        ...
        public override void Shoot(Player player, ModProjectile modproj, Vector2 tipPosition, Vector2 mousePosition, int Ring)
        {
            ...
            BaseConcentration con = player.GenerateConcentration<ConCallLightning>(Ring, TimeSpan * 60 * timeMulti);
            if (con != null)
            {
                con.Ring = Ring;
                (con as ConCallLightning).UseTime = 2;
            }
        }
