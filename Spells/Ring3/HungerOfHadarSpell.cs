...
{
    public class HungerOfHadarSpell : BaseSpell
    {
        ...
        public override DiceDamage BaseDamage => new DiceDamage(6, 2, DamageElement.Cold, CombatStat.Ring3Damage / 10f) + new DiceDamage(6, 2, DamageElement.Acid, CombatStat.Ring3Damage / 10f);
        ...
        {
            int protmp = player.NewMagicProj(mousePosition, Vector2.Zero, ModContent.ProjectileType<HungerOfHadarShowProj>(), player.GetDiceDamage(BaseDamage, InitialRing, Ring, RisingDamageAddition), 0, Ring);
        ...
        public override bool DrawLight(Player owner, int ring, Vector2 tipPos, ref float light, ref Color color, ref float scale, float miscTimer, bool HasShot)
        {
            ...
            DrawUtils.DrawRoSLaser(texRibbon, bars1, Color.LightYellow * light, 0.4f, 1f, miscTimer, DrawUtils.ReverseSubtract);
            EasyDraw.AnotherDraw(BlendState.Additive);
            Main.spriteBatch.Draw(LightTex, tipPos - Main.screenPosition, null, Color.White * light * 1.3f, miscTimer, LightTex.Size() / 2f, 0.06f * scale, SpriteEffects.None, 0);
            EasyDraw.AnotherDraw(DrawUtils.ReverseSubtract);
            Main.spriteBatch.Draw(LightTex, tipPos - Main.screenPosition, null, Color.White * light * 1.3f, miscTimer, LightTex.Size() / 2f, 0.03f * scale, SpriteEffects.None, 0);
            return false;
        }
    }
}
